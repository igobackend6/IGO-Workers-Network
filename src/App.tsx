...
export default function App() {
  ...
  return (
    <div 
      id="app-wrapper" 
      className="min-h-screen bg-gradient-mesh flex items-center justify-center p-2 sm:p-4 md:p-6 font-sans selection:bg-indigo-500/30 overflow-x-hidden"
    >
      
      {/* Main Glassmorphic Dashboard Console */}
      <div 
        id="main-glass-console" 
        className="w-full max-w-7xl glass-panel rounded-[32px] shadow-2xl flex flex-col md:flex-row overflow-hidden min-h-[92vh] border border-white/60"
      >
        
        {/* LEFT WORKSPACE SIDEBAR - Matching image perfectly */}
        <aside 
          id="workspace-sidebar" 
          className="w-full md:w-[84px] bg-white/30 backdrop-blur-md border-b md:border-b-0 md:border-r border-white/40 p-4 flex md:flex-col justify-between items-center shrink-0 gap-4"
        >
          {/* Top Button List */}
          <div className="flex md:flex-col gap-3.5 items-center">
            ...
            <button
              type="button"
              className="w-11 h-11 rounded-full bg-slate-900 hover:bg-slate-800 text-white flex items-center justify-center shadow-md hover:scale-105 transition-all cursor-pointer active:scale-95"
            ...
            <button
              type="button"
              className="w-11 h-11 rounded-full bg-white/80 hover:bg-white text-slate-600 flex items-center justify-center transition-all cursor-pointer shadow-sm active:scale-95 hover:rotate-45"
            ...
            <button
              type="button"
              className="w-11 h-11 rounded-full bg-white/80 hover:bg-white text-slate-600 flex items-center justify-center transition-all cursor-pointer shadow-sm active:scale-95"
              onClick={() => {
                setActiveTab('search');
                setTimeout(() => {
                  const inputEl = document.getElementById('ai-command-input');
          ...
          <div className="flex items-center gap-2 md:flex-col md:gap-0">
            {currentUser && (
              <button
                type="button"
                className="w-10 h-10 rounded-full bg-rose-50 hover:bg-rose-100 text-rose-600 flex items-center justify-center transition-all cursor-pointer shadow-sm md:mb-4 active:scale-95"
            ...
            <div className="w-12 h-12 rounded-2xl bg-slate-950 flex items-center justify-center text-white font-black text-lg tracking-tighter shadow-lg select-none hover:rotate-6 transition-transform">
              I
            </div>
          </div>
        </aside>
        ...
        <main className="flex-1 p-4 md:p-8 flex flex-col justify-between overflow-y-auto min-h-0 bg-white/20">
          ...
          <header id="console-header" className="flex justify-between items-center mb-6 md:mb-10 shrink-0">
            ...
            <div className="flex items-center gap-2 bg-white/60 px-4 py-2 rounded-2xl border border-white/40 cursor-pointer shadow-sm hover:bg-white/80 transition-all select-none">
              <Sparkles className="w-4 h-4 text-indigo-600 animate-pulse" />
              <span className="text-xs font-extrabold text-slate-800 tracking-tight">IGO Portal v2.6</span>
              <ChevronDown className="w-3.5 h-3.5 text-slate-500" />
            </div>

            {/* Center: Title info */}
            <div className="hidden sm:block text-center">
              <span className="text-[10px] text-indigo-600 font-extrabold tracking-widest block uppercase">Agri-Infrastructure System</span>
              <span className="text-xs font-black text-slate-800">Pan-India Labor Logistics</span>
            </div>

            {/* Right: Premium Status Badge */}
            <div className="flex items-center gap-2">
              <div className="hidden lg:flex flex-col text-right text-[10px] leading-tight mr-1">
                <span className="font-extrabold text-slate-700">Database Status</span>
                <span className="text-emerald-600 font-bold">● Active Sync</span>
              </div>
              <button
                type="button"
                className="px-4 py-2 bg-slate-950 hover:bg-slate-900 text-white text-xs font-extrabold rounded-full shadow-md flex items-center gap-1.5 transition-all cursor-pointer active:scale-95 border border-white/10"
                onClick={() => setShowInfoModal(true)}
              >
          ...
          <section id="greeting-section" className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8 shrink-0 relative">
            <div>
              <h1 className="text-3xl md:text-4xl font-black text-slate-900 tracking-tight leading-none font-display">
                Hi {currentUser ? currentUser.name.split(' ')[0] : 'Partner'},
              </h1>
              <p className="text-base md:text-lg text-slate-600 font-medium mt-1 font-sans">
                {currentUser 
            ...
            {!currentUser ? (
              ...
              <div className="w-full h-full animate-fadeIn" id="active-role-component">
                {currentUser.isSandbox && (
                  <div className="mb-6 p-4 bg-amber-50/90 backdrop-blur-sm border border-amber-200/80 rounded-2xl text-slate-800 text-xs flex flex-col sm:flex-row items-center justify-between gap-4 shadow-sm animate-pulse">
      ...
      {showInfoModal && (
        <div className="fixed inset-0 bg-slate-950/50 backdrop-blur-md z-50 flex items-center justify-center p-4 animate-fadeIn">
          <div className="w-full max-w-md bg-white border border-slate-100 rounded-3xl shadow-2xl p-6 relative">
            ...
            <div className="mt-4 space-y-3 text-xs text-slate-600 font-semibold leading-relaxed">
              <p>
                This terminal runs under fully verified <strong>Pan-India Infrastructure Governance</strong> protocols. All data transactions sync in real-time with our secure <strong>Google Cloud Firestore</strong> databases in the Mumbai cluster.
              </p>
              <div className="p-3 bg-indigo-50 border border-indigo-100 rounded-2xl">
                <span className="text-[10px] font-black text-indigo-700 block uppercase tracking-widest">Active Network Nodes</span>
            ...
            <div className="mt-6 flex justify-end">
              <button
                type="button"
                className="px-5 py-2 bg-slate-900 hover:bg-slate-800 text-white text-xs font-black uppercase tracking-wider rounded-xl cursor-pointer active:scale-95"
