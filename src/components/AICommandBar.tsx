      ...
      if (cleanText.includes('stale') || cleanText.includes('flagged') || cleanText.includes('old')) {
        ...
        deployments.forEach(d => {
          ...
          items.push({
            ...
            badgeColor: d.status === 'active' ? 'bg-emerald-100 text-emerald-700 border-emerald-200' : 'bg-amber-100 text-amber-700 border-amber-200'
          });
        });
        ...
        workers.forEach(w => {
          if (w.name?.toLowerCase().includes(cleanText) || w.homeDistrict?.toLowerCase().includes(cleanText) || w.skillCategory?.toLowerCase().includes(cleanText)) {
            items.push({
              ...
              badgeColor: w.status === 'active' ? 'bg-emerald-100 text-emerald-700 border-emerald-200' : 'bg-slate-100 text-slate-700 border-slate-200'
            });
          }
        });
  ...
  return (
    <div className="w-full mt-8 select-none" id="ai-command-bar-root">
      ...
      <div className="flex justify-between items-center text-xs text-slate-400 font-bold px-2 mb-2">
        ...
        <span className="font-medium">Powered by Assistant v2.6</span>
      </div>

      {/* 2. Glowing search input frame */}
      <div className="relative group rounded-2xl p-[1.5px] bg-gradient-to-r from-slate-200 via-indigo-200 to-slate-200 shadow-lg hover:shadow-indigo-100 hover:shadow-xl transition-all duration-300">
        <div className="flex items-center bg-white rounded-[15px] p-2 md:p-3 gap-2">
          ...
          <input
            id="ai-command-input"
            type="text"
            className="flex-1 bg-transparent border-none outline-none text-xs md:text-sm text-slate-800 placeholder-slate-400 font-medium font-sans"
          ...
          <div className="flex items-center gap-1 md:gap-2 shrink-0">
            ...
            <button
              ...
              type="button"
              className="w-8 h-8 md:w-9 md:h-9 rounded-full bg-slate-900 hover:bg-indigo-600 text-white flex items-center justify-center transition-all cursor-pointer shadow active:scale-95 shrink-0"
      ...
      <div className="flex flex-wrap gap-1.5 mt-3 justify-center sm:justify-start px-1" id="ai-quick-chips">
        <button
          type="button"
          className="px-2.5 py-1.5 bg-slate-50 border border-slate-200 hover:bg-indigo-50 hover:border-indigo-200 rounded-full text-[10px] font-bold text-slate-600 hover:text-indigo-700 transition-all cursor-pointer shadow-sm active:scale-95"
          onClick={() => {
      ...
      {isOpen && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fadeIn">
          <div className="w-full max-w-xl bg-white border border-slate-100 rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[80vh]">
            ...
            <div className="p-5 overflow-y-auto space-y-4 flex-1">
              ...
              <div className="p-4 bg-indigo-50/50 border border-indigo-100 rounded-2xl flex gap-3 items-start">
                ...
                <div>
                  <h5 className="text-[10px] font-black text-indigo-600 uppercase tracking-widest">Milo AI Assistant</h5>
                  <p className="text-xs text-slate-700 font-semibold mt-1 leading-relaxed">{aiMessage}</p>
                </div>
              </div>
                ...
                <div className="space-y-2 pt-2">
                  ...
                  <div className="space-y-2.5 max-h-[40vh] overflow-y-auto pr-1">
                    {results.map((item, idx) => (
                      <div key={item.id} className="p-3 bg-slate-50 border border-slate-100 rounded-2xl flex justify-between items-center gap-4 hover:border-indigo-100 transition-all">
                        <div className="flex items-center gap-3 min-w-0">
                          <div className="w-8 h-8 rounded-xl bg-indigo-100 text-indigo-700 flex items-center justify-center shrink-0">
                            {item.type === 'worker' && <User className="w-4 h-4" />}
                            {item.type === 'supervisor' && <User className="w-4 h-4 text-amber-600" />}
                            {item.type === 'deployment' && <Activity className="w-4 h-4 text-emerald-600" />}
                            {item.type === 'system' && <Activity className="w-4 h-4 text-blue-600" />}
                          </div>
            ...
            <div className="p-3 bg-slate-50 border-t border-slate-100 flex justify-end">
              <button
                type="button"
                className="px-4 py-1.5 bg-slate-900 hover:bg-slate-800 text-white text-xs font-black uppercase tracking-wider rounded-xl transition-all cursor-pointer active:scale-95"
                onClick={() => setIsOpen(false)}
              >
                Close Report
              </button>
